...
The injection plan is built once at decoration time.  Values are resolved
...
class _Plan:
    ...
    kwargs_safe: bool
    # Leading positional arguments that cannot reach an injected parameter.
    pos_guard: int
...
# pos_guard for a function nothing injectable can be passed positionally to.
...
def _fill(plan: _Plan, arguments: dict[str, Any]) -> None:
    """Resolve missing parameters into arguments, leaving explicit values alone."""
    ...
    if pending:
        ...
        for name in pending:
            ...
            if value is not _MISSING:
                ...
                # Nothing in the context to fill it, so let the parameter's own default apply.
                del arguments[name]
...
def _make_wrapper(
    ...
    def ensure_plan() -> _Plan:
        # A racing first call may build the plan twice, harmlessly, because plans are deterministic.
        nonlocal plan
        try:
    ...
    # Duplicated rather than shared, since a helper call costs about a third of the fast path.
