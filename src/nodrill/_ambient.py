...
class _AmbientContext:
    ...
    Set an attribute anywhere, read it anywhere below in the same thread or
    task; threads start empty, tasks inherit a snapshot and write locally.
    Writes are unscoped: nothing restores them on block exit, so use
    provider() for request-scoped state.

    Only dunders are defined, so no user attribute name can collide with the
    API.
    """
