...
A single ContextVar holds the registry mapping; providers swap in an updated
copy on enter and reset through the token on exit, so parents and sibling
tasks never observe another scope's keys.
"""
...
# Shared and never mutated; every write path copies first.
...
# Fallback factories registered with set_default().  Configuration, not
# per-context state: written at import time, read on use() misses.
_defaults: dict[type[Any], Callable[[], Any]] = {}

_MISSING = object()

# A tuple rather than `str | type`.  The union form is not a constant: it runs
# a BINARY_OP and allocates a UnionType every time it is evaluated, which on
# the use() path was most of the cost of a lookup.
_KEY_TYPES = (str, type)
...
class Namespace:
    ...
    # Namespaces made by provider() remember the name they were registered
    # under, for error messages and repr.  The name lives in a slot, mangled to
    # _Namespace__label, so it stays out of __dict__ and no prefilled value can
    # collide with it.
    ...
    if TYPE_CHECKING:
        # Declared so writes type-check under mypy strict.  Defining it for
        # real would route every write through Python instead of object's slot.
...
class _Provider(Generic[T]):
    ...
    # Nothing here awaits; the async protocol exists so `async with
    # provider(...)` reads the way callers expect it to in async code.
...
def provider(
    *args: Any, key: str | type[Any] | None = None, frozen: bool = False, **values: Any
    ...
    key= if one is given.  Same-key providers shadow outer ones; the outer
    ...
    if args:
        ...
        # provider(name="app", ...) spelling.  Reserved for string keys; with
        # a positional target, a name= keyword is an ordinary prefill value.
        try:
        ...
        if not isinstance(target, str):
            raise TypeError(
                "provider(name=...) expects a string name; pass instances positionally: "
                "provider(instance)"
            )
...
def use(key: Any, *, default: Any = _MISSING) -> Any:
    ...
    instance typed as that class.  On a miss: a set_default() factory, then
    the default argument, then NoProviderError.
    """
    ...
    try:
        ...
        # Unhashable key; the type check below turns it into the same message
        # any other wrong key kind gets.
        pass
...
def set_default(cls: type[T], factory: Callable[[], T] | None) -> type[T]:
    ...
    cached singleton).  None removes the registration.  Returns cls.
    """
...
def isolate() -> Iterator[None]:
    ...
    Providers and ambient attributes start empty; set_default() registrations
    made inside are rolled back.  Meant for test fixtures.
    """
