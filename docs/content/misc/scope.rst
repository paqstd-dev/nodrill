...
Passing arguments is the right default, and nodrill is built so it keeps working, because an explicitly passed argument always wins over injection.
...
A scoped context ends that, since the boundary sets the value once and the code that actually needs it asks for it directly.

The trade is real and worth stating.
A parameter is visible in the signature and a context lookup is not.
That is why the class-keyed form exists, why ``@inject`` puts the dependency back in the signature where you want it visible, and why the error on a miss goes to the trouble of listing what was active.
...
The two compose, and a container can own construction while nodrill carries the per-request scope to any depth of the call stack.
...
The exceptions are configuration and instrumentation, the ``set_default`` and :func:`~nodrill.declare` tables written at import time, the ledger :func:`~nodrill.debug` keeps while it is on with the fallback counter beside the declare catalogue, and the two switches :func:`~nodrill.annotate_exceptions` and :func:`~nodrill.set_codec` set at startup.
