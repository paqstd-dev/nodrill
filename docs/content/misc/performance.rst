...
the same read through a `frozen=True` provider                    116   5.0
the same read through a resolved `lazy` provider                  138   5.9
`use(Config)` on its own, without the call frame                  45    1.9
the same lookup through a `ref()` key                             145   6.3
bare `ContextVar.get()`, for reference                            16    0.7
`with provider(...)`, enter and exit                              632   27
the same with 8 providers already open                            720   31
`with provider(lazy(...))`, entered and exited unread             1607  69
`with provider(..., extend=True)`, over an 8-attribute namespace  1783  77
`wrap(fn)()`, per call into a thread                              540   23
================================================================  ====  ===
...
``frozen=True`` and :func:`~nodrill.lazy` add a proxy hop to every attribute the consumer touches.
