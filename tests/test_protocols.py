...
class TestInjectedSentinelLeaks:
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(lambda s: "x" in s, id="contains"),
            pytest.param(lambda s: s["x"], id="getitem"),
            pytest.param(int, id="int"),
            pytest.param(float, id="float"),
            pytest.param(lambda s: [0][s], id="index"),
            pytest.param(lambda s: s < 1, id="lt"),
            pytest.param(lambda s: s <= 1, id="le"),
            pytest.param(lambda s: s > 1, id="gt"),
            pytest.param(lambda s: s >= 1, id="ge"),
            pytest.param(lambda s: s + 1, id="add"),
            pytest.param(lambda s: 1 + s, id="radd"),
            pytest.param(lambda s: s - 1, id="sub"),
            pytest.param(lambda s: 1 - s, id="rsub"),
            pytest.param(lambda s: s * 1, id="mul"),
            pytest.param(lambda s: 1 * s, id="rmul"),
